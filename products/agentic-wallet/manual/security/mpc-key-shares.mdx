---
title: "MPC Model"
description: "MPC provides a mathematical guarantee, not a software promise — no single party holds your private key, and signing authority is split so none can act alone."
---

MPC provides a mathematical guarantee, not a software promise. In an MPC wallet, no single party — not you, not your agent, not Cobo — ever holds a complete private key. Signing authority is split into independent key shares held by separate parties. A threshold number must cooperate to sign a transaction; none can act unilaterally.
...
| **MPC (self-custodial)** | Split across parties | No single party can act without the others |
...
When you pair the wallet in the Human App, a second key group is created on your device:
...
**Why two groups?** The agent group handles automatic signing for transactions within your pact — your phone does not need to be involved for routine operations. The mobile group handles over-limit operations that exceed your pact's boundaries (where you must approve) and key recovery. This is what makes MPC a mathematical guarantee: even if your agent is compromised, the attacker cannot bypass the threshold requirement. Your pact defines the rules; MPC makes them unbreakable.
...
- [Key Generation and Resharing](/products/agentic-wallet/manual/security/keygen-resharing) — the cryptographic detail behind how key shares are created and refreshed
- [Key Share Recovery](/products/agentic-wallet/manual/security/key-share-recovery) — what to do when a share is lost
- [MPC Security Model](/products/agentic-wallet/manual/developer/mpc-security) — developer-focused technical details on the TSS protocol and threat model
