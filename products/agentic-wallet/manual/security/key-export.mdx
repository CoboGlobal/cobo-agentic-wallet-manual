...
To move funds from an exported Solana key, use the open-source `sol-rescue` tool, which signs Solana transactions directly from the scalar.
...
2. Run `sol-rescue`:
   ```bash
   sol-rescue --key <scalar_hex> --to <destination_wallet_address> --amount <SOL_amount>
   ```
