...
user to enter in the Cobo Cobo Agentic Wallet app. Share the code with the user through a
...
| `--src-addr` | No | — | Source address to transfer from. Defaults to the wallet's most recently created address for the chain |
...
| `--src-addr` | — | Source address to call from. Defaults to the wallet's most recently created address for the chain |
...
| `--source-address` | No | — | Address to sign with. Defaults to the wallet's most recently created address for the chain |
...
that the wallet owner must approve in the Cobo Cobo Agentic Wallet app. Re-run the command
...
Manage approval requests sent to the Cobo Agentic Wallet app for human review.
...
```bash
caw approval resolve <approval-id>
```
...
```bash
caw pact update-conditions <pact-id>
```
...
```bash
caw pact update-policies <pact-id>
```

| Option | Required | Description |
|---|---|---|
| `--policies` | Yes | JSON array of InlinePolicyCreate objects to replace existing policies (required). Each object has 'name', 'type', and 'rules' fields |
