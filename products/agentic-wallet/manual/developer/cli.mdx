...
# Allowed transfer
caw tx transfer --pact-id "$PACT_ID" --dst-address "$DST" --token-id SETH_USDC --amount 1 --chain-id SETH

# Oversized transfer — triggers policy denial
caw tx transfer --pact-id "$PACT_ID" --dst-address "$DST" --token-id SETH_USDC --amount 500 --chain-id SETH
```
