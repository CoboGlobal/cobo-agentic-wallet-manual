...
Supply `token` to use the pairing code from the Human App. If `token` is omitted, provisioning runs in agent-first mode — the response contains a temporary `activation_secret` that the human uses to complete binding.

<ParamField path="token" type="str | None" default="None">
  The pairing code from the Human App.
</ParamField>
