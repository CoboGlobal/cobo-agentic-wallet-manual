...
description: "Parse structured denials and implement self-correcting retry loops."
---
...
- **Pending approval** — the operation is allowed in principle but exceeds a threshold that requires the owner to sign off first. See [Set Policies](/developer/guides/set-policies) for how to configure approval thresholds.
...
```typescript
import { PolicyDeniedError } from 'cobo-agentic-wallet';
...
try {
  ...
  if (err instanceof PolicyDeniedError) {
    const limitValue = err.denial.details?.limit_value;
