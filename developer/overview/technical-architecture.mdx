...
flowchart TD
    ...
    subgraph Human["**Human (You)**"]
        im["Human App
        Claim · Approve · Policies · Notifications"]
    end

    subgraph WC["**Web Console**"]
        wc["Product Q&A · Onboarding guidance"]
    end
...
|---|---|
| **CLI (`caw`)** | Create wallets, execute transactions, generate claim codes, submit pacts — the agent-native interface |
| **Human App** | Claim wallets, manage Default Policy, approve pacts and over-limit transactions, freeze/revoke, review activity, back up keys |
| **Web Console** | Product exploration and onboarding Q&A via AI assistant — no wallet management |
...
| **Owner tools** | Claim wallet, manage Default Policy, approve/reject pacts, approve/reject over-limit transactions, freeze/unfreeze, review activity |
| **Web Console role** | Product Q&A and onboarding guidance only — no wallet management |
